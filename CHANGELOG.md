- fix: auto-delete empty groups when all children are deleted (v0.2.20, 56.7k loc)
...
- fix: loop label text no longer overlaps left browser panel — clips to canvas bounds like audio clip labels (55.3k loc)
- fix: monitoring latency reduced from ~150ms to ~20-37ms — smaller ring buffer, fixed buffer sizes, faster adaptive resampling, improved underrun handling (55.2k loc)
- fix: deleting/ungrouping a group now disables its monitoring and clears solo state (55.0k loc)
- docs: user guide updated — added Groups, Session Collaboration, Takes & Comping sections; added PaulStretch & Complex warp modes; fixed multi-select docs (54.9k loc)
